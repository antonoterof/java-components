...
public class MqttClientConnector implements IPubSubClient, MqttCallbackExtended
{
	...
	protected boolean subscribeToTopic(String topicName, int qos, IMqttMessageListener listener) {
		...
		if (qos < 0 || qos > 2) qos = ConfigConst.DEFAULT_QOS;
		...
		try {
			...
			_Logger.log(Level.SEVERE, "Subscribe failed: " + topicName, e);
			return false;
		}
	}
	...
	protected boolean unsubscribeFromTopic(String topicName) {
		...
		try {
			...
			_Logger.log(Level.SEVERE, "Unsubscribe failed: " + topicName, e);
			return false;
		}
	}
	...
	private class ActuatorResponseMessageListener implements IMqttMessageListener {
		...
		public void messageArrived(String topic, MqttMessage message) throws Exception {
			try {
				ActuatorData data = DataUtil.getInstance().jsonToActuatorData(new String(message.getPayload()));
				_Logger.info("Received ActuatorData response: " + data.getValue());
				if (dataMsgListener != null) {
					dataMsgListener.handleActuatorCommandResponse(resource, data);
				}
			} catch (Exception e) {
				_Logger.warning("Failed to convert payload to ActuatorData.");
			}
		}
	}
	...
	private class SensorDataMessageListener implements IMqttMessageListener {
		...
		public void messageArrived(String topic, MqttMessage message) throws Exception {
			try {
				SensorData data = DataUtil.getInstance().jsonToSensorData(new String(message.getPayload()));
				_Logger.info("Received SensorData: " + data.getName());
				if (dataMsgListener != null) {
					dataMsgListener.handleSensorMessage(resource, data);
				}
			} catch (Exception e) {
				_Logger.warning("Failed to convert payload to SensorData.");
			}
		}
	}
	...
	private class SystemPerformanceDataMessageListener implements IMqttMessageListener {
		...
		public void messageArrived(String topic, MqttMessage message) throws Exception {
			try {
				SystemPerformanceData data = DataUtil.getInstance().jsonToSystemPerformanceData(new String(message.getPayload()));
				_Logger.info("Received SystemPerformanceData: " + data.getCpuUtilization());
				if (dataMsgListener != null) {
					dataMsgListener.handleSystemPerformanceMessage(resource, data);
				}
			} catch (Exception e) {
				_Logger.warning("Failed to convert payload to SystemPerformanceData.");
			}
		}
